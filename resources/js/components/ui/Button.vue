<template>
  <div class="relative">
    <button
      class="w-full px-5 py-2.5 text-center text-white bg-blue-700 border border-blue-700 rounded-md
      text-sm mx-auto block hover:bg-blue-800 shadow-lg font-semibold dark:bg-blue-950
      dark:border-blue-950 dark:hover:bg-blue-900 dark:hover:border-blue-900 dark:text-gray-300
      disabled:bg-gray-500 disabled:border-gray-500 dark:disabled:dg-gray-500 dark:disabled:border-gray-500
      dark:disabled:hover:border-gray-500"
      :disabled="loading"
    >
      <slot/>
    </button>
    <div role="status" class="absolute top-0 flex h-full items-center right-0 mr-2" v-if="loading">
      <svg aria-hidden="true" class="inline w-5 h-5 mr-2 text-gray-400 animate-spin dark:text-gray-400 fill-gray-200"
